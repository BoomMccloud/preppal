...
| **3. Define tRPC API Routers** | Define the tRPC router procedures, including their input schemas and output types. **Do not implement the logic yet.** | Type-safe tRPC router definitions that the frontend team can immediately import to get autocompletion and type-checking for the API. |

...
describe('Protobuf Usage', () => {
  describe('Message Creation and Encoding/Decoding', () => {
    it('should create and wrap an AudioChunk message', () => {
      ...
      expect(message.audio_chunk).toBeDefined();
      expect(message.audio_chunk?.audio_content).toEqual(audioContent);
      expect(message.end_request).toBeUndefined();
    });
    ...
    it.fails('should encode and decode a ServerToClientMessage', () => {
      ...
      // 4. Decode it back
      const decodedMessage = interview_pb.preppal.ServerToClientMessage.decode(buffer);

      // 5. Verify the contents
      expect(decodedMessage).toBeDefined();
      expect(decodedMessage.transcript_update).toBeDefined();
      expect(decodedMessage.transcript_update?.speaker).toBe('AI');
      expect(decodedMessage.transcript_update?.text).toBe('Hello, how can I help you?');
      expect(decodedMessage.transcript_update?.is_final).toBe(true);
      expect(decodedMessage.audio_response).toBeUndefined();
    });
  });
});